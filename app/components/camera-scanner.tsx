...
function ToolButton({
    ...
    return (
        <button
            onClick={onClick}
            className="flex w-14 flex-col items-center gap-1.5 transition active:scale-90"
        >
            <span
                className={cx(
    ...
    return (
        <>
            <button
                ...
                onClick={onClose}
                className="absolute inset-0 z-30 bg-black/60 backdrop-blur-xs animate-fade-in"
            />
            ...
            <div className="px-6">
                <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button
                            onClick={onClose}
                            className="grid h-7 w-7 place-items-center rounded-full bg-zinc-800 text-zinc-400 hover:text-zinc-200"
                        >
...
export default function CameraScanner() {
    ...
    const maxScore =
        products.length > 0
            ? Math.max(...products.map((p) => p.environmentScore ?? 0))
            : -1;
    const bestEcoId =
        maxScore >= 0
                        ...
                        {products.length > 0 && (
                            <div className="absolute inset-0 z-10 pointer-events-none">
                                {products.map((p) => {
                                    ...
                                    const isSelected =
                                        p.detection_id === selectedId;

                                    const cxPct =
                                        (p.x / imageDims.width) * 100 -
                                        ((p.width / imageDims.width) * 100) / 2;
                                    const cyPct =
                                            ...
                                            {isBest && (
                                                <div
                                                    style={{
                                                        ...
                                                        zIndex: -1,
                                                        animation:
                                                            "goldCirclePulse 1.6s cubic-bezier(0.4, 0, 0.6, 1) infinite",
                                                    }}
                                                />
                                            )}
                                        </button>
